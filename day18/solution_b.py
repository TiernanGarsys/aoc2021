# Advent of Code 2021, Day 18, Solution B
...
def run_test(test: parse.Test):
    ...
    for i in range(len(test.numbers)):
        for j in range(len(test.numbers)):
            if i == j:
                continue
            
            potential_magnitude = magnitude(sum_numbers(test.numbers[i], test.numbers[j]))
            if potential_magnitude > max_magnitude:
                max_magnitude = potential_magnitude

    print('Answer: %s' % max_magnitude)
