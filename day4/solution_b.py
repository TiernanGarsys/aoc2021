# Advent of Code 2021, Day 4, Solution B
...
def run():
    ...
    for idx in range(len(draws)):
        ...
        if len(solved_boards) == len(boards) - 1:
            last_board = [b for b in boards if b not in solved_boards][0]
            break
    ...
    for idx in range(len(draws)):
        ...
        if check_board(last_board, draws_to_consider):
            solution = draws_to_consider
            break
   
    score = get_score(last_board, solution)
